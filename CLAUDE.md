...
- **Video editing**: `PUT /api/videos/:id` accepts `{ title, description }`. Validates title required (max 200 chars), description optional (max 5000 chars). Authorization: owner **or** admin can edit. Uses `updateVideo()` in `services/d1.ts` which also sets `updated_at = datetime('now')`.
- **Video deletion**: `DELETE /api/videos/:id` — owner **or** admin can delete (admin bypass via `getAdminUserId` check in both PUT and DELETE handlers).
...
- **Inline video editing**: On the video detail page, owners and admins see an edit button (`canManage = isOwner || isAdmin()`). `enterEditMode()` in `video.js` swaps the title `<h1>` and description div with form inputs in-place; Save calls `apiPut`, Cancel re-renders. Keyboard: Ctrl+Enter saves, Escape cancels.
- **Toast notifications**: `showToast()` in `admin.js` (module-private, not shared) creates a `.toast` element that slides down from page top and auto-dismisses after 2s. Used for settings save confirmation.
- **Media Session API**: `video.js` sets `navigator.mediaSession` metadata (title, artist, artwork) and action handlers (play/pause/seek) for OS-level playback controls. `clearMediaSession()` resets on navigation away.
...
- All `/api/videos/*` and `/api/settings/*` require auth (guests OK for GET); `/api/upload/*` and `/api/admin/*` also require non-guest; `PUT /api/videos/:id` requires non-guest; `PUT /api/settings` and `POST /api/admin/cleanup-orphans` additionally require admin. Admin can edit and delete any video (owner-or-admin check in both PUT and DELETE handlers).
